...
class Generator(torch.nn.Module):
    def __init__(self):
        ...
        self.deconv1 = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(CONFIG["Z_DIM"] + CONFIG["CC_DIM"] + CONFIG["DC_DIM"], 256, 4, 1, 0)
        )
...
class Discriminator(torch.nn.Module):
    def __init__(self):
        ...
        self.conv4 = torch.nn.Sequential(
            torch.nn.Conv2d(512, 1 + CONFIG["CC_DIM"] + CONFIG["DC_DIM"], 4, 1, 0)
        )
...
dataset = Cifar10DataSetForPytorch(root='/input/cifar10/', train=True, transform=transform)
