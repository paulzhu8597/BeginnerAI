...
class Generator(torch.nn.Module):
    def __init__(self):
        ...
        self.deconv5 = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(128, CONFIG["IMAGE_CHANNEL"], 4, 2, 1),
            torch.nn.Tanh()
        )
...
class Discriminator(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Sequential(
            torch.nn.Conv2d(CONFIG["IMAGE_CHANNEL"], 128, 4, 2, 1),
...
dataset = ImageFolder(root='/input/Faces/SquareImages/celeba',transform=transform)
...
g_optimizer = torch.optim.Adam(NetG.parameters(), lr=0.001, betas=(0.5, 0.999))
...
for epoch in range(1, CONFIG["EPOCH"] + 1):
    ...
    torchvision.utils.save_image(fake_images.data, "outputs/Face_%03d.png" % epoch, nrow=10)